...
{
	CharStatistics ch_stat;
	std::cout << "Input char(Enter、Ctrl+Z、Enter to quit):\n";
	int ch;
	while ((ch = std::cin.get()) != EOF)
		ch_stat.Insert(ch);

	std::cout << "first appears once: " << ch_stat.FirstAppearingOnce() << "\n\n";

	// boolalpha让输出流将bool解析成为true或者false, noboolalpha取消boolalpha流的格式标志
